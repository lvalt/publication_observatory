...
MIN_YEAR = 2023
API_DELAY = 3.0  # seconds between SS API calls
        ...
        for k in row:
            if re.search(r'level|taso|luokka', k, re.I):
                ...
                if v and v.strip():
                    try:
                        level = int(float(v))
                        if level in (0, 1, 2, 3):  # only valid JUFO levels
                            return {"level": level}
                    except: pass
    return {"level": None}
...
def build_snapshot(papers, snapshot_date=None):
    ...
    return {
        ...
        "avg_sjr": avg_sjr, "med_sjr": med_sjr,
    }
...
# Main pipeline
# ═══════════════════════════════════════════════════════
def run(author_xlsx, scimago_csv=None, jufo_csv=None, gs_csv=None,
    ...
    print(f"  SCImago matched: {sci_matched}, JUFO matched: {jufo_matched}, GS matched: {gs_matched}")
    ...
    if HAS_PANDAS:
        ...
        cols = ["author_name","author_id","title","year","publicationDate","venue","venue_issn","doi",
                "scimago_quartile","scimago_sjr","jufo_level","gs_h5_index","gs_h5_median","paper_id"]
    ...
    if template_path is None:
        for c in [Path("observatory_public.html"), Path(__file__).parent / "observatory_public.html"]:
            if c.exists(): template_path = str(c); break
    if not template_path:
        print("[6/6] ⚠ No template found — skipping HTML bake. Provide --template path.")
        return

    print(f"[6/6] Baking observatory HTML...")
    ...
    if not output_path:
        output_path = f"ai_doc_observatory_{today}.html"
...
Options:
  ...
  --template FILE   HTML template (observatory_public.html)
  --output FILE     Output HTML filename
  --history FILE    History JSON file (default: ai_doc_history.json)
  --min-year N      Minimum publication year (default: 2024)
